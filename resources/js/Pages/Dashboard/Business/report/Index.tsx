                    ...
                    <div className="w-full max-w-[1400px] mx-auto p-4 md:p-8 lg:p-10 flex flex-col gap-8">
                        ...
                        <ReportFiltersCard
                            ...
                            onExportPdf={() => {
                                ...
                                const type = templateMap[reportType] || "comprehensive";
                                let url = `/api/reports/download?template_type=${type}&organization_id=${organization?.id}`;
                                if (date) url += `&date=${date}`;
                                window.open(url, '_blank');
                            }}
                            onExportCsv={() => {
                                let url = `/api/reports/export-csv?organization_id=${organization?.id}`;
                                if (date) url += `&date=${date}`;
                                window.open(url, '_blank');
                            }}
                        />
